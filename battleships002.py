...
class Board:
    ...
    def add_ship(self, ship):  #вывод кораблей на доску
        ...
        for d in ship.dots:
            self.field[d.x][d.y] = "■"  #корабли помечаем таким знаком (в видимом поле игрока)
            self.busy.append(d)
    ...
    def __str__(self): #"конструктор" игрового поля
        res = ""
        res += "  | 1 | 2 | 3 | 4 | 5 | 6 |"
        for i, row in enumerate(self.field):
            res += f"\n{i + 1} | " + " | ".join(row) + " |"

        if self.hid:  #в скрытом поле игрока Ai изначально ставим только "."
            res = res.replace("■", ".")
        return res
    ...
    def shot(self, d):
        ...
        self.field[d.x][d.y] = "O"  #промах - добавление такого знака на поле
        print('Промазал!')
...
class Main:  #установка основных параметров самой игры
    ...
    def game(self):  #основной ход игры
        num = 0
        while True:
            print('-' * 27)
            print('Поле Игрока:')
            ...
            if num % 2 == 0:
                print('-' * 27)
                print('Ход Игрока - введите Ряд и Столбец через пробел')
                repeat = self.us.move()
            else:
                print('-' * 27)
                print('Ход AI:  ')
                repeat = self.ai.move()
            if repeat:
                num -= 1  #для того, чтобы не было передачи хода сопернику, после удачного выстрела
...
class Player:  #основные взаимодействия Игрока и Ai (противника)
...
class AI(Player): #Игрок Ai
    def ask(self):
        d = Dot(randint(0, 5), randint(0, 5))
        print(f'Ai пошёл так: {d.x + 1} {d.y + 1}')
        return d
